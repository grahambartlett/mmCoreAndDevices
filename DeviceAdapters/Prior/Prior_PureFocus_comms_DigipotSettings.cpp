...
int PureFocus850AutoFocus::GetStoredOffsetUm(const long index, double& value)
{
	int ret = DEVICE_OK;
	...
	if (ret == DEVICE_OK)
	{
		...
		if (ret == DEVICE_OK)
		{
			...
			if (ret == DEVICE_OK)
			{
				...
				{
					// Convert value from steps/s to um/s, where 25600 steps = 1mm
					value = atof(answer.c_str()) * (1000.0 / 25600.0);
				}
			}
		}
	}
